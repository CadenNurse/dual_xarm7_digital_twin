...
void MTCTaskNode::setupPlanningScene()
{
  ...
  // position the cylinder at (0.5, -0.25, 0.0) in the world frame
...
{
  task_ = createTask(); // create the MTC task

  try
  {
    task_.init(); // initialize the task, which sets up the stages and solvers
  }
  ...
  if (!task_.plan(5)) // generate a plan for the task, allowing up to 5 seconds for planning
  {
    RCLCPP_ERROR_STREAM(LOGGER, "Task planning failed");
    return;
  }
  task_.introspection().publishSolution(*task_.solutions().front()); // publish the first solution for visualization in RViz (not needed if you don't care about visualization)
  ...
  return; // all occurs via an action server interface with Rviz plugin
}
...
#pragma GCC diagnostic pop
  ...
  cartesian_planner->setMaxVelocityScalingFactor(1.0); // use the cartesian planner which requries the following properties to be set
