...
namespace uf_robot_hardware
{
    ...
    void UFRobotSystemHardware::_init_ufactory_driver(void)
    {
        ...
        if (!clean_prefix.empty() && clean_prefix.back() == '_') {
        ...
        node_ = rclcpp::Node::make_shared(clean_prefix + "ufactory_driver", node_options);
        hw_node_ = rclcpp::Node::make_shared(clean_prefix + "ufactory_robot_hw", node_options);
